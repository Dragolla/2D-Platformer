...
public class Player : MonoBehaviour
{
    ...
    {
        //This is basically a function for us to control the bools and ints that will transition animations. This may be changed in the future if/when we implement animation trees or layermasks. -Daniel


        if (myrigidbody.velocity.x == 0)
        {
            PlayerAnimator.SetBool("Idle", true);
            PlayerAnimator.SetBool("ShortMovement", false);
        }
        if (myrigidbody.velocity.x > 0 || myrigidbody.velocity.x < 0)
        {
            PlayerAnimator.SetBool("Idle", false);
            PlayerAnimator.SetBool("ShortMovement", true);
        }
